...
because each attention module is still cubic in the input. For simplicity, we
reparametrize the weights and write only $Q_l$ for the product $Q_l^\top K_l$ in
the rest of this section. The key result of the work by Oswald et al. and Ahn et
al. [[3,4]](#3) is that the output generated by a linear transformer can
equivalently be obtained by executing gradient descent on the input $Z_0$.
...
and generalize the setting to mulitple layers, i.e. $L>1$. Inspired by the
parameters in (2), Ahn et. al [[3]](#3) have restricted the parameter space to
...
It turns out that $L$-layer linear transformers with parameters as in (3) can
implement various standard optimization methods. Ahn et. al [[3]](#3) have shown
that the prediction $y_l^{(n+1)}$ of the $l$-th layer can be described as
follows: The prediction $y_l^{(n+1)} = - \langle x^{(n+1)},
\theta_l^{\text{gd}} \rangle$ relies on the sequence of parameters
${\theta_l^{\text{gd}}}$ defined as
...
Having seen the key result of the paper [[3]](#3), one might wonder if trained
...
In this way, we give the transformer more representational power and it turns
out that transformers can learn more sophisticated algorithms beyond
preconditioned gradient descent. Assuming once again normal distributed data
$x^{(i)} \sim \mathcal{N}(0, \Sigma)$ and $\theta_* \sim \mathcal{N}(0,
\Sigma^{-1})$ and defining the parameter space $\mathcal{S} := \left\\{
\\left(\\{A_l\\}\_{l=1}^L, \\{B\\}\_{l=1}^L \\right) \ | \
A_l = a_l \Sigma^{-1}, \ B_l = b_l I_d \ \ \forall l \right\\}$, Ahn et al.
[[3]](#3) have shown that it holds:
...
by Oswald et al. [[4]](#4). This algorithm is based on gradient descent but
introduces an interative curvature correction $x_i \leftarrow (b_l I-
XX^\top)x_i$ for all inputs $x_i$. Experiments have shown that in our setup GD++
outperforms plain gradient descent and describes the behavior of trained
